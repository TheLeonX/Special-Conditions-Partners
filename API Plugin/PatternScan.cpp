...
__int64 PatternScan::Scan(std::string pattern, __int64 startIndex, bool backwards)
{
	...
	__int64 base = plugin::moduleBase;
	if (startIndex != 0) base = startIndex;

	__int64 total = plugin::moduleBase + plugin::moduleLength - totalBytes;
	...
	{
		for (__int64 x = startIndex; x >= plugin::moduleBase + 0xC00; x--)
		{
			bool isCorrect = true;
			for (int y = 0; y < totalBytes; y++)
			{
